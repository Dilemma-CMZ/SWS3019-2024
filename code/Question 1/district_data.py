...
if __name__ == '__main__':
    df1 = pd.read_csv('house_belonging.csv')
    ...
    df1.to_csv('district1.csv')

    df2 = pd.read_csv('mall_belonging.csv')
    df2 = df2.groupby('belonging').count()
    df2 = df2.iloc[:, :1]
    df2.rename(columns={'Unnamed: 0': 'mall_cnt'}, inplace=True)
    df2.info()
    df2.to_csv('district2.csv')

    df1 = pd.read_csv('district1.csv')
    df2 = pd.read_csv('district2.csv')
    ...
    df_merged.info()
    df_merged.to_csv('district.csv', index=False)
